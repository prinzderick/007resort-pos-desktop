...
public abstract class ScreenViewModel : ObservableObject
{
    ...
    public static string Describe(Exception ex) => ex switch
    {
        ...
        ApiException { Code: "pending_collection_exists" } => "Already collected by the waiter: confirm or reject it in 'Collected by waiters' instead of taking payment again.",
        ApiException { Code: "over_collection" } => "That is more than is left to collect on this bill (part of it is already collected by a waiter).",
        ApiException { Code: "order_billed" } => "The bill is printed, so the order is frozen. Reopen the bill (supervisor) to change it.",
        ApiException { Code: "collections_pending" } => "Money was collected on this bill. Confirm or reject the collection before reopening the bill.",
        ApiException { Code: "supervisor_required" } => "A supervisor must authorise this: tick 'A supervisor is here' and let them sign in.",
        ApiException { Code: "self_confirmation_forbidden" } => "You cannot confirm money you collected yourself. Ask another cashier or a supervisor.",
        ApiException { Code: "payment_state_invalid" } => "This collection was already decided (confirmed, rejected or expired). The list has been refreshed.",
        ApiException { Code: "auto_confirm_only" } => "This payment is confirmed automatically by the payment provider; a person cannot confirm it.",
        ApiException { Code: "already_received" } => "This handover was already received. The list has been refreshed.",
        ApiException { Code: "self_receipt_forbidden" } => "You cannot receive your own cash handover.",
        ApiException { Code: "self_signoff_forbidden" } => "The person who received the cash cannot sign off its variance. Ask another supervisor.",
        ApiException { Code: "handover_state_invalid" } => "This handover cannot be changed in its current state. The list has been refreshed.",
        ApiException { Code: "order_not_billed" or "bill_not_printed" } => "Print the bill first.",
